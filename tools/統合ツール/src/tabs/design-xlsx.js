...
export async function runAdvancedDesignExporter(params = {}) {
  ...
  function buildFieldUsageMap(ctx) {
    ...
    Object.entries(fields).forEach(([code, f]) => {
      ...
      if (expr) extractCodesFromExpr(expr, fields).forEach((c) => { if (c !== code) add(c, `計算式「${f.label || code}」`); });
      ...
      if (f.type === 'SUBTABLE' && f.fields) {
        for (const [sc, sf] of Object.entries(f.fields)) {
          ...
          if (se) extractCodesFromExpr(se, fields).forEach((c) => { if (c !== sc) add(c, `計算式「${sf.label || sc}」`); });
        }
      }
    });
  ...
  try {
    ...
    if (selectedSheets.has('fields')) {
      const fieldHeaders = [
        ...
        'ルックアップ設定', '関連レコード設定', '計算式', '依存/参照', '使用箇所', '説明'
      ];
