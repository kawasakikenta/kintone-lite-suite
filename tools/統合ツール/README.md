...
差分比較の判定規則は [差分比較ロジック説明](../../docs/diff-comparison-logic.md) を参照してください。比較方向、取得失敗時の扱い、配列の対応付け、無視・正規化、上限、HTML / Excel出力の違いを実装と対応付けて説明しています。
...
lite版では、差分確認時の比較元・比較先の設定とrevisionを保持し、実行前に再取得して照合します。確認後の変更、取得途中のrevision不一致、取得失敗や一部未取得がある対象は、書き込み前に中断して差分の再確認を案内します。対象セクションを減らした場合は残した項目を照合します。

反映中は、自分の更新応答で返ったrevisionを次の書き込みへ引き継ぎます。revision競合や更新後revisionの欠落を検知した場合は「エラーで停止」の設定にかかわらず中断し、残りを未実行として表示します。バックアップを選んだ場合は、対象設定を完全に取得・照合できてから保存を開始します。
...
「CSV出力」と「レコード管理」のCSV出力は、サブテーブルの明細を自動で別CSVに分けます。追加の出力設定は不要です。

| 対象 | 保存形式と内容 |
| --- | --- |
| 1アプリ・テーブルなし | 従来どおりCSVを1ファイルで保存 |
| 1アプリ・テーブルあり | `records.csv`、`tables/<テーブルコード>.csv`、`manifest.json` を1つのZIPで保存 |
| 複数アプリ | アプリ別フォルダに親CSV・明細CSV・`manifest.json` を格納し、全体を1つのZIPで保存。テーブルなしのアプリも `records.csv` と `manifest.json` を格納 |

親の `records.csv` は1レコードにつき1行です。テーブル列は従来どおり行数の要約を保持し、明細の実際の値は `tables/` 内のCSVに出力します。テーブルがある場合は、親CSVの先頭に紐付け用の `$id` を追加します。

明細CSVはテーブルの1行につき1行で、列は次の順序です。

| 列 | 内容 |
| --- | --- |
| `$id` | 親レコードID。親CSVの `$id` と紐付けるキー |
| `$rowId` | kintoneのテーブル行ID |
| `$rowIndex` | 親レコード内でのテーブル行の順番（1始まり） |
| 子フィールドコード | テーブル内の各項目の値 |

複数のテーブルはそれぞれ別CSVに出力します。対象レコードがある場合、空のテーブルもヘッダーだけのCSVを作成します。複数アプリの一部で取得に失敗した場合は、成功したアプリをZIPに保存し、失敗一覧をZIP全体の `manifest.txt` と完了メッセージに表示します。指定したファイル名の末尾の `.csv` / `.zip` は、実際の保存形式に合わせて調整します。

レコードバックアップでも同じ親CSVと明細CSVを保存し、`records.json` にテーブルを含む取得データを保持します。CSVの出力情報はバックアップの `manifest.json` の `csvExport` に記録します。

取得結果にテーブルが含まれないレコードは、親CSVのテーブル列を空欄にし、`manifest.json` の `warnings`（バックアップでは `csvExport.warnings`）と完了メッセージで未取得を通知します。取得できた空テーブルの `0行` と区別します。

これらのCSVは閲覧・集計用で、kintoneのCSV取込や本ツールのCSV取込へそのまま戻せる互換形式ではありません。本ツールのCSV取込はサブテーブルに対応していません。添付フィールドはCSVへファイル名だけを出力します。ファイル本体も必要な場合はバックアップの「添付ファイルも保存」を選んでください。このオプションはテーブル内の添付にも対応しています。単独の「添付DL」もテーブル内のフィールドを名前から選択できます。取込用の列が必要な場合は「CSV取込ひな形を作成」を使ってください。
...
│   │   ├── enrich.ts         # 差分行の表示用補強（日本語理由, 改名候補, 関連設定）
...
| タブ | 主な用途 | 代表機能 |
|------|----------|----------|
| 差分比較 | 2アプリ間の設定差分を把握 | BEFORE/AFTER固定レーン、全体/セクション別レビュー進捗、確認して次へ、レビュー状態JSON保存/読込、集中/コンパクト表示、IME対応の軽量検索、モバイルドロワー、フィルターチップ、日本語優先カード、長文プレビュー、顧客共有向けの簡潔なレビューExcel、HTML出力 |
...
| タブ | 主な用途 | 代表機能 |
|------|----------|----------|
| レコード管理 | 実データのメンテナンス | 重複・未入力検査、CSV取込ひな形、ステータス更新、添付DL、CSV、バックアップ、アプリ間コピー |
