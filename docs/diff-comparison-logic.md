...
- 単一候補のJSONは、入力欄のApp IDとJSON内App IDが違っても、その1件を採用する
- `fetchedAt`がないJSONには読込時刻を補うため、元データの実取得日時とは限らない
- 読込ファイル自体のサイズ上限は設けていない

App IDは単一JSONの完全性を証明する照合値ではありません。ファイル名、取得元、App ID、取得日時を人が確認してください。
...
候補が複数ある場合は推測せず、通常の追加・削除を残します。名前だけが変わった場合、改名行は `_displayOnly` なので実差分件数は0です。
...
| 内容モード | 収録内容 | 利用できる機能 |
|---|---|---|
| 差分行のみ `diffOnly` | 差分行の左右値。全設定スナップショットは収録しない | 通常レビュー、共有 |
| 比較設定込み `withCompared` | 選択セクションの比較設定も収録 | フィールド詳細、比較元/先JSON、反映JSON |

`diffOnly` は全設定を載せないという意味で相対的に安全です。匿名化や全面マスキングではありません。変更されたJS/CSS本文、プラグイン設定、その他の機密値が差分行にあれば収録されます。共有先と保管場所を確認してください。
...
- 通常は「比較概要」「変更一覧」の2シートだけを出力する
- 取得失敗、一部未検証、件数上限がある場合だけ「確認できなかった範囲」を概要の直後に追加する
- 追加、削除、変更、並び順変更を重複しない客観件数で表示する
- 同一行と表示専用の参考行は収録しない
- 変更一覧は11列とし、確認状況、対応方針、担当者、コメントだけを入力欄にする
- 差分ID、存在状況、技術パス、原文JSON、長文、URL、検索語、アプリ等の数値IDを顧客向けブックへ収録しない
- 権限、通知、プラグイン、JS/CSS設定は分類単位に集約し、変更前後の値を「詳細は安全のため非表示」へ置き換える
- その他のオブジェクト値も件数等の安全な要約へ置き換える
- 概要には比較結果、完全性、比較日時、比較対象、掲載範囲、絞り込み・条件調整の有無、技術値省略件数、分類別件数を残す
...
Lite版Excelは同一行を収録せず、JS/CSS・プラグイン・権限・通知の実値も収録しません。一方、HTMLの `diffOnly` には変更された機密値が残る場合があります。HTMLとExcelは収録範囲が異なるため、同じ共有安全性とはみなしません。
...
取得失敗、本文未検証、エンジン上限、HTML収録上限のいずれかがある場合、誤った反映JSONを作らないよう機能を無効にします。
