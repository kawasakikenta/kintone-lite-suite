...
export async function runBatchProcessStandalone(opts, setStatus) {
  ...
  for (let i = 0; i < ids.length; i += 100) {
    ...
    const body = { app: appId, records: batch.map((id) => { const r = { id, action }; if (assignee) r.assignee = assignee; return r; }) };
...
export async function runRecordCopyStandalone(opts, setStatus) {
  ...
  const clean = records.map((rec) => {
    ...
    for (const [k, v] of Object.entries(rec)) {
      ...
      if (v.type === 'SUBTABLE') {
        out[k] = { value: v.value.map((sr) => { const c = {}; for (const [sk, sv] of Object.entries(sr.value)) c[sk] = { value: sv.value }; return { value: c }; }) };
      } else {
        out[k] = { value: v.value };
      }
    }
