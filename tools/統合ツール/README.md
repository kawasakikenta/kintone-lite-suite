...
lite版は「対象選択 → 差分確認 → 最終確認 → 結果」の4段階です。反映先のアプリ名とID、追加・変更・削除・保持の内訳を確認します。標準では反映先だけのフィールド・配置・一覧・グラフ・アクションを保持します。取得失敗は自動除外せず、選択されたままなら実行を止めます。

実行直前に設定とrevisionを再取得して確認時と照合し、バックアップJSONの保存を開始してから書き込みます。反映中は更新応答のrevisionを引き継ぎ、エラー時は中断します。最終確認は条件変更や戻る操作で解除され、本番公開はアプリ設定画面で行います。

詳しい反映範囲と制約は [プレビュー反映の使い方](../../docs/preview-reflection.md) を参照してください。
