...
`;
  ...
  function mountJsconfigLitePanel() {
    ...
    srcApp.setAttribute("aria-label", "取得元アプリID");
    srcGuest.setAttribute("aria-label", "取得元ゲストスペースID");
    tgtApp.setAttribute("aria-label", "反映先アプリID");
    tgtGuest.setAttribute("aria-label", "反映先ゲストスペースID");
    ...
    installLiteWorkflow(panel, {
      setup: [cardFetch.card, cardJson.card, cardApply.card, batchDetails.details],
      results: [cardPreview.card],
      resultActions: ["fetch"],
      beforeRun: (actionId) => {
        if (actionId === "fetch") {
          previewBox.replaceChildren();
          previewBox.classList.add("kus-lp__panel-html--empty");
        }
      },
      actions: [
