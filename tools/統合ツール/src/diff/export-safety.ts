...
/**
 * 共有される出力で、省略した原文が同じかを照合するための SHA-256。
 * Web Crypto は非同期 API のため、同期でワークブックを組み立てる経路でも使える小さな実装を置く。
 */
...
export function contentIdentifier(text: string): string {
  return `SHA256-${sha256Hex(text).slice(0, 16).toUpperCase()}`;
}
