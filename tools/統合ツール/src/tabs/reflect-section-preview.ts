...
function computeMapDiff(beforeMap, afterMap) {
  const keys = new Set([...Object.keys(beforeMap || ({} as any)), ...Object.keys(afterMap || ({} as any))]);
  const rows = [];
  for (const key of keys) {
    const bf = (beforeMap || ({} as any))[key];
    const af = (afterMap || ({} as any))[key];
    if (!bf && af) rows.push({ key, status: 'added', before: null, after: af, changes: [] });
      ...
      const bfKeys = new Set(Object.keys(bf || ({} as any)));
      const afKeys = new Set(Object.keys(af || ({} as any)));
      const allProps = new Set([...bfKeys, ...afKeys]);
      const changes = [];
      allProps.forEach((prop) => {
        if (!deepEqual((bf || ({} as any))[prop], (af || ({} as any))[prop])) {
          changes.push({ prop, before: (bf || ({} as any))[prop], after: (af || ({} as any))[prop] });
        }
      });
...
export function initSectionPreviewEditor(ui, setStatus) {
  ...
  function loadFromDiffBundles(options: any = {}) {
  ...
  function applySectionChange(nextSectionKey, options: any = {}) {
  ...
  root.__sectionPreviewApi = {
    setSection(nextSectionKey, options: any = {}) {
    ...
    if (modalAct) {
      try {
        ...
        if (modalAct === 'saveItemJson' && st.modal?.kind === 'itemJson') {
          const raw = (root.querySelector('[data-spe-modal-input="itemJson"]') as HTMLInputElement | null)?.value || '';
          const parsed = JSON.parse(raw);
          pushUndo();
          if (isMapSection(st.sectionKey)) {
            const items = deepClone(getAfterItems() || ({} as any));
        ...
        if (modalAct === 'saveFullJson' && st.modal?.kind === 'fullJson') {
          const raw = (root.querySelector('[data-spe-modal-input="afterJson"]') as HTMLInputElement | null)?.value || '';
          const parsed = JSON.parse(raw);
          pushUndo();
          st.after = deepClone(parsed);
          closeModal();
        ...
        if (modalAct === 'saveAddItem' && st.modal?.kind === 'addItem') {
          const newKey = (root.querySelector('[data-spe-modal-input="newKey"]') as HTMLInputElement | null)?.value?.trim();
          const raw = (root.querySelector('[data-spe-modal-input="newItemJson"]') as HTMLInputElement | null)?.value || '';
