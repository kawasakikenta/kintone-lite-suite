...
${selected.summary().map(([key, value]) => `${key}: ${value}`).join("\n")}`;
  ...
  var EXPORT_SHEET_DEFS = [
    ...
    { key: "fields", label: "項目定義", default: true, cat: "basic" },
    { key: "layout", label: "フォームレイアウト", default: true, cat: "basic" },
    { key: "views", label: "一覧", default: true, cat: "basic" },
    { key: "reports", label: "グラフ", default: true, cat: "basic" },
    { key: "status", label: "プロセス管理", default: true, cat: "process" },
    { key: "statusMatrix", label: "遷移マトリクス", default: true, cat: "process" },
    { key: "appAcl", label: "アプリ権限", default: true, cat: "acl" },
    { key: "recordAcl", label: "レコード権限", default: true, cat: "acl" },
    { key: "fieldAcl", label: "フィールド権限", default: true, cat: "acl" },
    { key: "customize", label: "JS/CSSカスタマイズ", default: true, cat: "customize" },
    { key: "actions", label: "アクション", default: true, cat: "customize" },
    { key: "plugins", label: "プラグイン", default: true, cat: "customize" },
    { key: "webhook", label: "Webhook", default: true, cat: "customize" },
    { key: "genNotif", label: "通知(一般)", default: true, cat: "notif" },
    { key: "recNotif", label: "通知(レコード)", default: true, cat: "notif" },
    { key: "remNotif", label: "通知(リマインダー)", default: true, cat: "notif" },
    { key: "adminNotes", label: "管理者メモ", default: true, cat: "other" },
    { key: "dependencies", label: "フィールド依存関係", default: true, cat: "other" }
  ];
  var EXPORT_SHEET_CATEGORIES = [
    { key: "basic", label: "基本" },
    { key: "process", label: "プロセス" },
    { key: "acl", label: "権限" },
    { key: "customize", label: "カスタマイズ・連携" },
    { key: "notif", label: "通知" },
    { key: "other", label: "その他" }
  ];
  var EXPORT_SHEET_PRESETS = [
    { id: "all", label: "すべて", keys: null },
    { id: "basic", label: "基本のみ", keys: ["summary", "fields", "layout", "views", "reports"] },
    { id: "acl", label: "権限のみ", keys: ["appAcl", "recordAcl", "fieldAcl"] },
  ...
  var EXPORT_FIELD_TYPE_CATEGORIES = [
    { key: "deco", label: "レイアウト・装飾" },
    { key: "field", label: "入力・システム項目" }
  ];
  var rememberedFieldTypeExclusion = null;
  function loadPersistedFieldTypeExclusion() {
    return rememberedFieldTypeExclusion ? new Set(rememberedFieldTypeExclusion) : null;
  }
  function persistFieldTypeExclusion(excluded) {
  ...
  async function loadSheetLibOnce() {
    ...
    try {
      await loadScriptLocal(SHEETLIB_PRIMARY_URL);
      return { styled: true };
    } catch {
      await loadScriptLocal(SHEETLIB_FALLBACK_URL);
      return { styled: false };
    }
  }
  async function loadJSZipOnce() {
  ...
  function showSheetSelectionDialog(defs, title = "📊 エクスポート設定") {
    return new Promise((resolve) => {
      ...
      const optionalTotal = defs.filter((d) => !d.required).length;
      const usedCats = EXPORT_SHEET_CATEGORIES.filter((c) => defs.some((d) => d.cat === c.key));
      const groupsHtml = usedCats.map((cat) => {
        const items = defs.filter((d) => d.cat === cat.key);
        const rows = items.map((d) => {
          const checked = remembered ? remembered.has(d.key) : d.default;
          return `<label style="display:flex;align-items:center;gap:6px;margin:3px 0;font-size:13px;cursor:${d.required ? "default" : "pointer"};"><input type="checkbox" value="${escapeHtmlAttr(d.key)}" ${checked ? "checked" : ""} ${d.required ? "disabled" : ""}>${escapeHtmlAttr(d.label)}${d.required ? " (必須)" : ""}</label>`;
        }).join("");
        return `<div style="margin-bottom:10px;"><div style="font-size:11px;font-weight:bold;color:#2E5C8A;letter-spacing:.04em;margin:2px 0 4px;border-bottom:1px solid #e2e8f0;padding-bottom:2px;">${escapeHtmlAttr(cat.label)}</div>${rows}</div>`;
      }).join("");
      const presetBtns = EXPORT_SHEET_PRESETS.map(
        (p) => `<button type="button" class="kex-preset" data-preset="${escapeHtmlAttr(p.id)}" style="font-size:11px;padding:4px 10px;border:1px solid #c2d1de;border-radius:999px;background:#eef4f8;color:#24506f;cursor:pointer;">${escapeHtmlAttr(p.label)}</button>`
      ).join("");
      ...
      const box = overlay.querySelector('[role="dialog"]');
      const optionBoxes = () => Array.from(overlay.querySelectorAll('#kex-sheet-options input[type="checkbox"]'));
      const selectableBoxes = () => Array.from(overlay.querySelectorAll('#kex-sheet-options input[type="checkbox"]:not([disabled])'));
      const countEl = overlay.querySelector("#kex-count");
      const warnEl = overlay.querySelector("#kex-warn");
      const exportBtn = overlay.querySelector("#kex-export");
      const refresh = () => {
        const checked = optionBoxes().filter((cb) => cb.checked).length;
        countEl.textContent = `選択中 ${checked} / ${optionalTotal}`;
        const empty = checked === 0;
        exportBtn.disabled = empty;
        exportBtn.style.opacity = empty ? "0.5" : "1";
        exportBtn.style.cursor = empty ? "not-allowed" : "pointer";
        warnEl.textContent = empty ? "出力するシートを1つ以上選択してください。" : "";
      };
      ...
      const applyPreset = (keys) => {
        selectableBoxes().forEach((cb) => {
          cb.checked = keys ? keys.includes(cb.value) : true;
        });
        refresh();
      };
      overlay.querySelectorAll(".kex-preset").forEach((btn) => {
        btn.onclick = () => {
          const preset = EXPORT_SHEET_PRESETS.find((p) => p.id === btn.dataset.preset);
          if (preset) applyPreset(preset.keys);
        };
      });
      overlay.querySelector("#kex-select-all").onclick = () => applyPreset(null);
      overlay.querySelector("#kex-select-none").onclick = () => applyPreset([]);
      const fieldTypeBoxes = () => Array.from(overlay.querySelectorAll("input.kex-ftype"));
      const setFieldTypes = (checked) => fieldTypeBoxes().forEach((cb) => {
        cb.checked = checked;
      });
      overlay.querySelector("#kex-ftype-all")?.addEventListener("click", () => setFieldTypes(true));
      overlay.querySelector("#kex-ftype-none")?.addEventListener("click", () => setFieldTypes(false));
      const close = (result) => {
        doc.removeEventListener("keydown", onKeydown, true);
        if (overlay.parentNode) overlay.parentNode.removeChild(overlay);
        resolve(result);
      };
      const doCancel = () => close(null);
      const doExport = () => {
        ...
        fieldTypeBoxes().forEach((cb) => {
          if (!cb.checked) excludedTypes.add(cb.value);
        });
        persistFieldTypeExclusion(excludedTypes);
        close(selected);
      };
      overlay.querySelector("#kex-cancel").onclick = doCancel;
  ...
  async function runAdvancedDesignExporter(params = {}) {
    ...
    function getExporterOverlayZIndex2() {
      const main = getToolDocument().getElementById(TOOL_ID);
      const raw = main ? Number(window.getComputedStyle(main).zIndex) : NaN;
      const base = Number.isFinite(raw) ? raw : 2147483646;
      return String(Math.min(2147483647, Math.max(2e9, base + 1)));
    ...
    const UI = {
      ...
      show(msg, totalSteps = 10) {
        ...
        if (!el) {
          el = doc.createElement("div");
          el.id = UI.id;
          Object.assign(el.style, { position: "fixed", top: "0", left: "0", width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.7)", zIndex: getExporterOverlayZIndex2(), display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", color: "#fff", fontSize: "16px", fontFamily: '"Meiryo", sans-serif' });
          doc.body.appendChild(el);
        }
    ...
    const UtilsX = {
      ...
      formatEntity: (entity) => {
        ...
        const typeMap = { USER: "ユーザー", GROUP: "グループ", ORGANIZATION: "組織", FIELD_ENTITY: "フィールド値", CREATOR: "作成者", MODIFIER: "更新者", LOGIN_USER: "ログインユーザー", ALL: "全員", EVERYONE: "全員" };
      ...
      formatDefaultValue: (dv) => {
        const fnLabel = (code) => ({ "LOGIN_USER()": "ログインユーザー", "PRIORITY_ORGANIZATION()": "優先する組織" })[code] || code;
        if (dv == null) return "";
        if (Array.isArray(dv)) {
          if (dv.length > 0 && typeof dv[0] === "object") return dv.map((i) => i.name || fnLabel(i.code) || JSON.stringify(i)).join("、");
          return dv.join("、");
        }
        if (typeof dv === "object") {
          if (dv.type === "NUMBER") return String(dv.value || "");
          return dv.name || fnLabel(dv.code) || JSON.stringify(dv);
        }
        return String(dv);
      },
    ...
    try {
      ...
      const fieldTypeExclusion = params.fieldTypeExclusion instanceof Set ? new Set(params.fieldTypeExclusion) : loadPersistedFieldTypeExclusion() || /* @__PURE__ */ new Set();
      const isFieldTypeExcluded = (type) => fieldTypeExclusion.has(String(type || ""));
      if (progressLabel) {
        const prefix = `${progressLabel} `;
        const origShow = UI.show.bind(UI);
        const origUpdate = UI.update.bind(UI);
        UI.show = ((msg, totalSteps) => origShow(`${prefix}${msg}`, totalSteps));
        UI.update = ((msg, step) => origUpdate(`${prefix}${msg}`, step));
      }
      ...
      if (selectedSheets.has("fields")) {
        ...
        const walkForFields = (rows, groupLabel) => {
          ...
          for (const row of safeRows) {
            ...
            if (row?.type === "SUBTABLE") {
              if (row.code) {
                orderedItems.push({ kind: "FIELD", code: row.code, groupLabel });
                seenFieldCodes.add(row.code);
              }
              continue;
            }
            const items = Array.isArray(row?.fields) ? row.fields : [];
      ...
      if (selectedSheets.has("reports") && reports?.reports) {
        ...
        const rows = Object.entries(reports.reports).sort(([, a], [, b]) => (Number(a.index) || 0) - (Number(b.index) || 0)).map(([name, r]) => [
          ...
          Array.isArray(r.groups) ? r.groups.map((g) => `${g.code || ""}${g.per ? `(${ENUM_LOOKUP(GROUP_PER_LABEL, g.per)})` : ""}`).join("、") : "",
          Array.isArray(r.aggregations) ? r.aggregations.map((a) => {
            const typeLabel = ENUM_LOOKUP(AGGREGATION_TYPE_LABEL, a.type) || "";
            return a.code ? `${typeLabel}（${a.code}）` : typeLabel;
          }).join("\n") : "",
          Array.isArray(r.sorts) ? r.sorts.map((sr) => `${ENUM_LOOKUP(REPORT_SORT_BY_LABEL, sr.by)}${sr.order ? ` ${UtilsX.formatSort(sr.order)}` : ""}`.trim()).join("\n") : "",
          UtilsX.formatFilterCond(r.filterCond),
          r.periodicReport?.active ? `あり${r.periodicReport?.period?.every ? `（${ENUM_LOOKUP(PERIODIC_EVERY_LABEL, r.periodicReport.period.every)}）` : ""}` : "-"
        ]);
      ...
      if (selectedSheets.has("actions") && actions) {
        ...
        const rows = entries.map((a, i) => [
          ...
          Array.isArray(a.entities) && a.entities.length ? a.entities.map((e) => UtilsX.formatEntityDetailed(e)).join("\n") : "すべてのユーザー",
          Array.isArray(a.mappings) ? a.mappings.map((m) => `${m.srcType === "RECORD_URL" ? "レコードのURL" : m.srcField || m.sourceField || "-"}→${m.destField || "-"}`).join("\n") : ""
        ]);
      ...
      const renderNotifSheet = (title, payload, kind) => {
        ...
        if (kind === "reminder") {
          const headers2 = ["No.", "通知先", "基準フィールド", "通知タイミング", "絞り込み条件", "通知内容"];
          ...
          const sheetTitle2 = payload?.timezone ? `${title}（タイムゾーン: ${payload.timezone}）` : title;
          appendSheet(
            title,
            { ...buildSimpleAOA(sheetTitle2, headers2, rows2), pageSetup: { orientation: "landscape", printTitleRows: 2 } },
            { description: "リマインダー通知の設定一覧" }
          );
          return;
        }
        if (kind === "perRecord") {
          const headers2 = ["No.", "通知先", "絞り込み条件", "通知内容"];
          const rows2 = list.map((n, i) => [
            i + 1,
            formatNotificationTargets(n, fieldLabelMap),
            UtilsX.formatFilterCond(n.filterCond || ""),
        ...
        const sheetTitle = payload?.notifyToCommenter != null ? `${title}（コメント投稿者への通知: ${payload.notifyToCommenter ? "有効" : "無効"}）` : title;
        appendSheet(
          title,
          { ...buildSimpleAOA(sheetTitle, headers, rows), pageSetup: { orientation: "landscape", printTitleRows: 2 } },
...
⚠ ${UI.failedAPIs.length}件のAPI取得に失敗しました` : "";
  ...
  function nowStampZip(d = /* @__PURE__ */ new Date()) {
  ...
  async function runBatchDesignExportXlsxZip(params) {
    ...
    for (let i = 0; i < appIds.length; i++) {
      ...
      try {
        ...
        const { wb, filename, appName, failedAPIs: failedAPIs2 } = ret;
        const doc2 = getToolDocument();
        const innerOverlay = doc2.getElementById("kintone-exporter-overlay");
        if (innerOverlay && innerOverlay !== doc2.getElementById("kintone-exporter-overlay")) {
        }
        ...
        const buf = window.XLSX.write(wb, { bookType: "xlsx", type: "array", cellStyles: true });
        zip.file(uniqueName, buf);
        if (failedAPIs2 && failedAPIs2.length) batchUi.addFailedAPIs(`App ${appId}`, failedAPIs2);
...
`;
  ...
  function mountDesignLitePanel() {
    ...
    const importedBundles = /* @__PURE__ */ new Map();
    const appNameLookup = () => {
      const map = {};
      importedBundles.forEach((b, id) => {
        const n = extractAppNameFromBundle(b);
        if (n) map[id] = n;
      });
      return map;
    };
    importFile.addEventListener("change", () => liteRun(panel, "設定JSONを読み込み中…", async () => {
      ...
      for (const b of bundles) {
        ...
        const result = appTable.putApp(appId, String(b?.guestId || ""), { appName: extractAppNameFromBundle(b) });
    ...
    bJson.addEventListener("click", () => {
      if (!requireFirstApp()) return;
      liteRun(panel, "設計書 JSON 生成中…", async () => {
        await runDesignExportStandalone("json", source(), (m, e) => panel.setStatus(m, e ? "err" : "busy"));
      });
    });
    ...
    bXlsx.addEventListener("click", () => {
      if (!requireFirstApp()) return;
      liteRun(panel, "Excel 生成中…", async () => {
        await runDesignExportXlsxStandalone({ ...source(), appNameLookup: appNameLookup() }, (m, e) => panel.setStatus(m, e ? "err" : "busy"));
      });
    });
    ...
    bDiff.addEventListener("click", () => {
      ...
      if (all.length < 2) {
        panel.setStatus("差分には対象アプリ表に2行以上のアプリIDが必要です", "warn");
        return;
      }
  ...
  runOnKintonePage(mountDesignLitePanel);
})();
