  ...
  async function runSettingsExportStandalone(mode, opts, setStatus) {
    ...
    if (mode === "zip") {
      ...
      const entries = bundles.map((bundle, i) => {
        ...
        const failedSections2 = scopes.filter((key) => bundle.sections?.[key]?._fetchError);
        return {
          ...
          failedSections: failedSections2.map((key) => ({
            key,
...
`;
  ...
  function mountSettingsExportLitePanel() {
    ...
    installLiteWorkflow(panel, {
      setup: [cardTarget.card, cardScope.card],
      results: [summary],
      beforeRun: () => {
        summary.replaceChildren();
        summary.classList.add("kus-lp__panel-html--empty");
      },
      actions: [
