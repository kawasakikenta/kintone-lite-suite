...
Lite 版は、取得定義にあるkintoneアプリ設定18セクションをすべて選択できます。取得定義と画面の選択肢は同じ一覧から生成し、新しいセクションが画面から漏れないようにしています。既定で18セクションすべてを選択し、意図せず未選択にした設定を「差分なし」と誤解することを防ぎます。

- アプリ設定、アプリ情報（名称・ラベル）、フォーム設定
- フィールド、レイアウト
- ビュー、グラフ
- プロセス、アクション
- JS/CSS、プラグイン
...
- ZIPの先頭に `000_一括比較結果.xlsx` を収録し、登録順、比較元・比較先、結果、取得状態、未取得・打切りの範囲、追加・削除・変更・並び順変更の変更対象件数、アプリ名の不一致、各個別Excel名を一覧できるようにする。少なくとも1件の個別Excelを生成できた場合は、個別Excelの生成に失敗した行も一覧へ残す
...
- 表の上へ比較件数、差分あり・差分なしの件数、変更対象の総件数、一部未取得とExcel生成失敗の件数を1行で載せる
- 掲載件数は各個別Excelの「変更対象件数」（「変更対象一覧」と同じ変更対象単位）と同じ数え方をそろえる
...
`diffOnly` は全設定を載せないという意味で相対的に収録範囲が狭いだけです。変更されたJS/CSS本文、プラグイン設定、その他の機密値が差分行にあれば収録されます。Excelにも同じ差分値を加工せず収録するため、どちらを共有する場合も差分値と共有先・保管場所を確認してください。長文を表示用に省略する箇所には SHA-256 の先頭128ビットを `SHA256-` 付きの内容識別子として付与し、別の出力の原文と同一かを照合できるようにします。
...
- 「比較概要」と「変更一覧」を索引として常設し、実差分があるAPIだけを `API_01_アプリ一般設定` から `API_20_カテゴリー設定` のAPI別シートへ分ける。「比較概要」（取得状態の注意があるときは「確認できなかった範囲」）の直後に、利用者向けの説明シート「このファイルの見方」を常設し、シートの構成、確認の手順、変更対象一覧・変更一覧・機能別シートの列の読み方、値の表記をまとめる。説明シートは比較結果や技術パスを含まず、比較概要の「シートの使い分け」からも案内する。対応APIを特定できない差分は `API_99_API未特定` へまとめ、差分がないAPIの空シートは作らない
...
- 概要の追加、削除、変更、並び順変更は、「変更対象一覧」と同じ変更対象単位の重複しない客観件数で表示する。フィールド・一覧などの対象自体の追加・削除だけを追加・削除に数え、設定項目単位の明細件数とは「明細」の表記で区別する
...
- 概要には比較結果、完全性、比較日時、比較対象、掲載範囲、絞り込み、比較から除外した条件の具体名、変更対象一覧と同じ対象単位の変更対象件数、API別の差分明細件数を残す。比較結果は取得が一部未完了でも確認できた範囲の変更対象件数を併記し、完全性には未取得・打切りになった設定領域の名称を添える。実差分0件を「変更なし」と言い切るのは比較が完全なときだけとする
