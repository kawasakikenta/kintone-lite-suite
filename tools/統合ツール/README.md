...
これらのCSVは閲覧・集計用で、kintoneのCSV取込や本ツールのCSV取込へそのまま戻せる互換形式ではありません。本ツールのCSV取込はサブテーブルに対応していません。添付フィールドはCSVへファイル名だけを出力します。ファイル本体も必要な場合はバックアップの「添付ファイルも保存」を選んでください。このオプションはテーブル内の添付にも対応しています。単独の「添付DL」はテーブル外の添付フィールドが対象です。
