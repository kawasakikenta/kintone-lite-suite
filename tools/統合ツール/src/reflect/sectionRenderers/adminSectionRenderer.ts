...
import {
  createDefaultSectionRenderer,
  SectionRenderer,
  SectionRendererInput,
  SectionRendererHelpers,
  SectionRendererRow
} from './defaultRenderer.js';
...
function changedPropSet(row: SectionRendererRow | null | undefined, helpers: SectionRendererHelpers): Set<string> {
  const out = new Set<string>();
  if (Array.isArray(row?.changes) && row!.changes!.length) {
    row!.changes!.forEach((ch: any) => { if (ch?.prop) out.add(String(ch.prop)); });
    return out;
  }
  const bf = row?.before && typeof row.before === 'object' ? row.before : {};
  const af = row?.after && typeof row.after === 'object' ? row.after : {};
  const keys = new Set<string>([...Object.keys(bf), ...Object.keys(af)]);
  keys.forEach((key) => {
...
interface AdminCardOptions {
  changed?: Set<string>;
}

function renderAdminCard(title: string, rows: AdminCardRow[], options: AdminCardOptions = {}): string {
...
function renderViewSettingsCard(view: any, row: SectionRendererRow, helpers: SectionRendererHelpers): string {
...
function renderLayoutCard(layout: any): string {
  const rows = Array.isArray(layout) ? layout : [];
  const body = rows.map((line: any, idx: number) => {
    const fields = Array.isArray(line?.fields) ? line.fields : [];
    const names = fields.map((f: any) => f?.code || f?.type || '-');
...
function renderProcessCard(process: any): string {
  ...
  const stateRows = states.map((s: any) => `<tr><td>${esc(s?.name || '-')}</td><td>${esc(s?.assignee?.type || '-')}</td></tr>`).join('');
  const actionRows = actions.map((a: any) => `<tr><td>${esc(a?.name || '-')}</td><td>${esc(a?.from || '-')}</td><td>${esc(a?.to || '-')}</td></tr>`).join('');
...
function renderNotificationsCard(notifications: any): string {
  const list = Array.isArray(notifications) ? notifications : [];
  const rows = list.map((item: any, idx: number) => {
    const cond = item?.filterCond || item?.condition || '-';
    const recipients = Array.isArray(item?.recipients) ? item.recipients.map((r: any) => r?.entity?.code || r?.entity?.type || r?.code || '-').join(', ') : '-';
...
function renderAclCard(rights: any): string {
  const list = Array.isArray(rights) ? rights : [];
  const body = list.map((r: any, idx: number) => `<tr><td>${idx + 1}</td><td>${esc(r?.entity?.code || r?.entity?.type || '-')}</td><td>${esc((r?.includeSubs ? '配下含む' : '単体'))}</td><td>${esc(normalizeText(r?.appEditable || r?.recordViewable || r?.editable || r?.viewable))}</td></tr>`).join('');
...
type PreviewAdapter = (val: any, row: SectionRendererRow, helpers: SectionRendererHelpers) => string;

const PREVIEW_ADAPTERS: Record<string, PreviewAdapter> = {
...
export function createAdminSectionRenderer(): SectionRenderer {
  const fallback = createDefaultSectionRenderer();

  function renderPreview(input: SectionRendererInput & { sectionKey?: string }): string {
    const { row, sectionKey, helpers } = input;
    const adapter = sectionKey ? PREVIEW_ADAPTERS[sectionKey] : undefined;
