...
function sideAppLabel(bundle: DiffXlsxBundle | undefined, fallback: string): string {
...
function rowItemLabel(
  ...
  try {
    ...
    // 人向けの label があれば、技術パスへフォールバックする前に採用する。
  }
...
function summarizeListComplexValue(
  ...
  if (!summary && !Array.isArray(value)) {
    ...
    const facts = Object.entries(value)
      ...
      .slice(0, 4)
      .map(([key, item]) => `${labels[key]}: ${humanizeListScalar(item)}`);
...
function buildSummarySheet(
  ...
  const sheetRows: (string | number | null)[][] = [
    ...
    ['使い方', fieldCount
      ? '①「フィールド差分要約」で対象を確認　②「フィールド差分詳細」で変更前後を確認　③「差分一覧」で確認状態・担当者・コメントを入力'
      : '①概要で判定と完全性を確認　②「差分一覧」で変更前後を確認　③黄色い3列に確認状態・担当者・コメントを入力', '', rows.length ? '③ レビュー入力へ' : ''],
    ['', '', '', ''],
    ['セクション別集計', '', '', ''],
...
function fieldSummaryStateLabel(
  summary: DiffXlsxFieldSummary,
  model: DiffXlsxFieldModel,
  sourceBundle?: DiffXlsxBundle,
  targetBundle?: DiffXlsxBundle
