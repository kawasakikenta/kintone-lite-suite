...
`diffOnly` は全設定を載せないという意味で相対的に収録範囲が狭いだけです。匿名化や全面マスキングではありません。変更されたJS/CSS本文、プラグイン設定、その他の機密値が差分行にあれば収録されます。顧客へ渡す場合は顧客向けExcelを使用し、HTMLを共有する場合は差分値と共有先・保管場所を確認してください。
...
- 取得失敗、一部未検証、実差分の件数上限がある場合だけ「確認できなかった範囲」を概要の直後に追加する
