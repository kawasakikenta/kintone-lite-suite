...
function computeDiff(before, after) {
  ...
  for (const code of keys) {
    ...
    if (!bf && af) rows.push({ code, status: 'added', before: null, after: af, changes: [] });
      ...
      const props = new Set([...Object.keys(bf || ({} as any)), ...Object.keys(af || ({} as any))]);
      const changes = [];
      props.forEach((prop) => {
...
function collectFieldFromForm(form, fallback) {
  const type = (form.querySelector('[data-rpp-field="type"]') as HTMLInputElement | null)?.value || fallback?.type || 'SINGLE_LINE_TEXT';
  const code = ((form.querySelector('[data-rpp-field="code"]') as HTMLInputElement | null)?.value || fallback?.code || '').trim();
  const label = ((form.querySelector('[data-rpp-field="label"]') as HTMLInputElement | null)?.value || fallback?.label || '').trim();
...
function renderFieldFormRows(draft) {
  ...
  return keys.map((key) => {
    ...
    if (key === 'options' && value && typeof value === 'object') {
      const lines = (Object.values(value) as any[]).sort((a: any, b: any) => a.index - b.index).map((o: any) => o.label).join('\n');
...
export function initReflectPreviewPlayground(ui, setStatus) {
  ...
  const loadFromDiffBundles = (options: any = {}) => {
    ...
    st.before = sourceProps || ({} as any);
    st.after = targetProps || ({} as any);
    st.filter = 'all';
    st.view = 'diff';
    st.expanded = new Set(
    ...
    if (modalAct) {
      try {
        ...
        if (modalAct === 'saveFieldJson' && st.modal?.kind === 'fieldJson') {
          const raw = (root.querySelector('[data-rpp-modal-input="fieldJson"]') as HTMLInputElement | null)?.value || '';
        ...
        if (modalAct === 'savePairJson' && st.modal?.kind === 'pairJson') {
          const b = (root.querySelector('[data-rpp-modal-input="beforeJson"]') as HTMLInputElement | null)?.value || '';
          const a = (root.querySelector('[data-rpp-modal-input="afterJson"]') as HTMLInputElement | null)?.value || '';
          pushUndo();
        ...
        if (modalAct === 'confirmAction' && st.modal?.kind === 'confirm') {
          const payload = st.modal.payload || ({} as any);
  ...
  root.addEventListener('change', (ev: Event) => {
    ...
    const sel = (ev.target as Element | null)?.closest('[data-rpp-field="type"]') as HTMLSelectElement | null;
    if (!sel) return;
    const form = root.querySelector('.rpp-field-grid') as HTMLElement | null;
    const currentCode = (form?.querySelector('[data-rpp-field="code"]') as HTMLInputElement | null)?.value || st.modal.draft.code;
    const currentLabel = (form?.querySelector('[data-rpp-field="label"]') as HTMLInputElement | null)?.value || st.modal.draft.label;
    const type = sel.value || 'SINGLE_LINE_TEXT';
    st.modal.draft = { type, code: currentCode, label: currentLabel, ...deepClone(DEFAULT_PROPS[type] || ({} as any)) };
    render();
  });
