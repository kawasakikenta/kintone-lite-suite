...
export function buildDiffHtml(sourceBundle, targetBundle, rows, scopes, ignoreKeys, options: any = {}) {
  ...
  const emptyFactTitle = reportMeta.incompleteComparison
    ? '確認できた差分は0件です'
    : reportMeta.exportMode !== 'all' ? 'この出力範囲の差分は0件です' : '差分は見つかりませんでした';
  const emptyFactNote = reportMeta.incompleteComparison
    ? '未取得・未検証の範囲があるため、一致とは判断できません'
  ...
  const formatEnvironment = (meta: any) => [
