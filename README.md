...
- 統合版本体 (`tools/統合ツール.js`) は廃止し、個別ツールの単機能スクリプト (`tools/*.js`) を正規の公開エントリポイントとします。
...
- `tools/統合ツール/build.js` 実行時に、`tools/*.js` の単機能スクリプトを、各機能に必要なコードのみを同梱した**軽量 lite バンドル**として自動生成します（手編集せず、`src/` 側を修正して再ビルドします）。
