...
describe('diff/engine', () => {
  ...
  describe('アプリアクションの並び順オプション', () => {
    ...
    it('選択時はフィールドの関連付けの順序差分も除外する', () => {
      const mappingA = { srcField: '請求金額', destField: '翌月請求金額' };
      const mappingB = { srcField: '担当者', destField: '引継担当者' };
      const source = makeBundle({ actionSettings: { actions: {
        '案件転記': { name: '案件転記', index: '0', mappings: [mappingA, mappingB] }
      } } });
      const target = makeBundle({ actionSettings: { actions: {
        '案件転記': { name: '案件転記', index: '0', mappings: [mappingB, mappingA] }
      } } });

      expect(computeDiffRows(source, target, ['actionSettings'], '').rows.length).toBeGreaterThan(0);
      expect(computeDiffRows(source, target, ['actionSettings'], '', {
        normalizationPresetState: { actionOrder: true }
      }).rows).toHaveLength(0);
    });
  });
