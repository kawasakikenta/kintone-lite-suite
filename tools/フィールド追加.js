(function (global) {
  ...
  function タブを開く() {
    ...
    const ランチャー = rootDoc.querySelector('[data-launch-feature][data-launch-tab="field"]');
    if (ランチャー) {
      ランチャー.dispatchEvent(new MouseEvent('click', { bubbles: true }));
      return true;
    }
    const タブ = rootDoc.querySelector('.tab[data-tab="field"]');
    if (タブ) {
  ...
  function タブ起動待機(retries) {
    if (タブを開く()) return;
    if (retries <= 0) {
      console.warn('[統合ツール] タブ切り替えに失敗しました:', 'field');
      return;
    }
